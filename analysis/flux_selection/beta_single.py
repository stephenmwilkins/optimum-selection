...
cat = h5py.File('../../data/beta_lum.hdf5', 'r')
...
for i in range(len(selection_filters)):
    ...
    for add_noise in [False]:
        ...
        fig.savefig(f'figures/beta_all_{i}_{label}.png') # probaby need to use png due to so many points
        ...
        fig.savefig(f'figures/beta_selection_{i}_{label}.png')
        fig.clf()
